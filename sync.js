...
/**
 * Compara dois valores (números, strings, arrays ou objetos) da mesma forma
 * que JSON.stringify(a) === JSON.stringify(b) faria, mas SEM se importar
 * com a ordem das chaves dentro de cada objeto.
 *
 * Por que existe (CORREÇÃO do bug "app fica se atualizando sozinho o tempo
 * todo"): a comparação em aplicarDadosDaNuvem() usava JSON.stringify direto
 * nos dois lados. Isso parece seguro, mas o Firestore NÃO garante que
 * devolve os campos de um objeto/mapa (ex.: cada receita em receitasExtras,
 * ou o objeto de notas) na mesma ordem em que foram gravados — só garante
 * o CONTEÚDO. Resultado: mesmo quando nada mudou de verdade, os dados que
 * "voltam" da nuvem podiam ter as chaves em ordem diferente da versão
 * local, JSON.stringify via essa ordem como texto diferente, e o código
 * concluía (errado) que algo mudou — gravando tudo de novo e dando
 * location.reload(). Como isso podia acontecer a cada favoritar, dar nota,
 * salvar ou excluir receita (sempre que o Firestore confirma a escrita),
 * a página ficava recarregando sozinha o tempo todo, tornando o site
 * inutilizável.
 * O que faz: ordena as chaves de cada objeto (recursivamente, inclusive
 * dentro de arrays de objetos) antes de transformar em texto — assim a
 * comparação só acusa diferença quando o CONTEÚDO realmente mudou, nunca
 * só por causa da ordem.
 * Quando: usado só dentro de aplicarDadosDaNuvem(), na hora de decidir se
 * recarrega a página ou não.
 *
 * @param {*} valor
 * @returns {*} o mesmo valor, com as chaves de todo objeto (em qualquer
 *   nível) reordenadas alfabeticamente
 */
function ordenarChavesRecursivamente(valor) {
  ...
  if (valor && typeof valor === "object") {
    ...
    Object.keys(valor).sort().forEach(chave => {
      chavesOrdenadas[chave] = ordenarChavesRecursivamente(valor[chave]);
    });
 ...
 * checou snapshot.metadata.hasPendingWrites antes. A comparação usa
 * ordenarChavesRecursivamente() nos dois lados (veja o comentário dessa
 * função acima) para não confundir "ordem diferente" com "conteúdo
 * diferente" — é isso que corrige o bug do site recarregando sozinho sem
 * parar.
...
function aplicarDadosDaNuvem(dados) {
  ...
  const normalizadoParaComparar = JSON.stringify(ordenarChavesRecursivamente(normalizado));
  const localParaComparar = JSON.stringify(ordenarChavesRecursivamente(coletarDadosLocais()));

  if (normalizadoParaComparar === localParaComparar) {
    return; // nada realmente mudou (ex.: eco da nossa própria escrita já confirmada, ou só a ordem das chaves)
  }
